...
describe('baseline verse ratings', () => {
  it('provides one rating for every verse in the Bible', () => {
    ...
    assert.ok(ratings.every((rating) => rating.score >= 6 && rating.score <= 10));
  });

  it('keeps the rubric anchors buffered from 6 to 10', () => {
    ...
    assert.equal(getBaselineVerseRating(50, 4, 13), 10);
    assert.equal(getBaselineVerseRating(24, 29, 11), 10);
    assert.equal(getBaselineVerseRating(45, 8, 28), 10);
    assert.equal(getBaselineVerseRating(49, 4, 32), 9);
    assert.equal(getBaselineVerseRating(33, 6, 8), 9);
    assert.equal(getBaselineVerseRating(59, 1, 2), 9);
    assert.equal(getBaselineVerseRating(58, 11, 1), 9);
    assert.equal(getBaselineVerseRating(21, 3, 1), 8);
    assert.equal(getBaselineVerseRating(60, 5, 7), 8);
    assert.equal(getBaselineVerseRating(44, 2, 42), 7);
    assert.equal(getBaselineVerseRating(13, 1, 1), 7);
    assert.equal(getBaselineVerseRating(2, 38, 24), 6);
    assert.equal(getBaselineVerseRating(7, 19, 29), 6);
    assert.equal(getBaselineVerseRating(9, 15, 3), 6);
  });
